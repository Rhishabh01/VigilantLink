...
Each external scan (GSB, RDAP, SSL) has an individual timeout. Timeouts set a `*_timed_out` flag and return safe defaults. The orchestrator applies uncertainty penalties based on timeout count and heuristic context.
